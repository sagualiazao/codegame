...
<!-- 用于登录的弹出窗口 -->
    <div>
        <el-form :model="loginForm" :rules="loginRules" ref="loginForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item>
                <el-button id="find-pass" type="text" @click="Findpass_change">找回密码</el-button>
            </el-form-item>
            <el-form-item>
...
export default {
    ...
    methods: {
        ...
        submitForm: function (formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$parent.$parent.$store.commit('changeMenu', 'menu-bar')
                } else {
                    alert('邮箱或密码输入不正确!')
                    return false
                }
        ...
        Findpass_change () {
            this.$parent.$parent.$store.commit('FindpassWindow', true)
            this.$parent.$parent.$store.commit('SigninWindow', false)
        }
    }
}
