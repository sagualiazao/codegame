...
describe('EditorBase.vue', () => {
    ...
    it('isSameFormat函数执行成功', () => {
        ...
        expect(vm.isSameFormat(/^\s*var\s+[A-Za-z]\w*\s*=\s*(\d+|[A-Za-z]\w*)\s*$/, 'var xx=')).to.equal(false)
        expect(vm.isSameFormat(/^\s*var\s+[A-Za-z]\w*\s*=\s*(\d+|[A-Za-z]\w*)\s*$/, 'var xx=dd')).to.equal(true)
        expect(vm.isSameFormat(/^\s*var\s+[A-Za-z]\w*\s*=\s*(\d+|[A-Za-z]\w*)\s*$/, 'var xx=3d')).to.equal(false)
        expect(vm.isSameFormat(/^\s*var\s+[A-Za-z]\w*\s*/, '  var xx')).to.equal(true)
        expect(vm.isSameFormat(/^\s*var\s+[A-Za-z]\w*\s*/, 'var _x')).to.equal(false)
    ...
    it('indexInCommandLibrary', () => {
        ...
        expect(vm.indexInCommandLibrary('   var xx=dd')).to.deep.equal('30')
        expect(vm.indexInCommandLibrary('varxx=dd')).to.equal(false)
        expect(vm.indexInCommandLibrary('varxx=')).to.equal(false)
        expect(vm.indexInCommandLibrary('var x')).to.equal('31')
        expect(vm.indexInCommandLibrary('var 3')).to.equal(false)
        expect(vm.indexInCommandLibrary('var _')).to.equal(false)
    ...
    it('getSafeCode', () => {
        ...
        expect(vm.getSafeCode('var x=dd')).to.equal('var x=dd;')
        expect(vm.getSafeCode('  x = 3')).to.equal('  x = 3;')
